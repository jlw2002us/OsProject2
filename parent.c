 ...
 int main(int argc, char* argv[])
  {
    ...
    while(signal_interrupt == false){
     ...
     if (ShmPTR->childQueue  < s){
         ...
         ShmPTR->childQueue++;//printf("%d",ShmPTR->childQueue);
         if ((childID = fork()) == -1) {
           perror("fork error");
           exit(EXIT_FAILURE);
        }
        ...
        else{ wait(NULL);   //wait for a child to finish
             i = i - 1;ShmPTR->childQueue--;}
    ...
    if (signal_interrupt == true) break;
     ...
     signal_interrupt = true;}
      for(i = 0; i<  ShmPTR->childQueue; i++){//wait for any
